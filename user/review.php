...
if($_GET['productId']){
    ...
    $getProduct = "SELECT *, cartintegration.status AS 'cartStatus', cartintegration.update_time AS 'statusUpdateDate'  FROM cartintegration LEFT JOIN product ON cartintegration.productId = product.id WHERE cartintegration.cartIntegrationId = '$cartIntegrationId' AND cartintegration.userId = '$userId'";
    ...
    if($resultGetProduct ->num_rows>0){
      while($row = $resultGetProduct ->fetch_assoc()){
        ...
        $review_product_id = $row['id'];
        $review_product_name = $row['name'];
        $review_product_price = $row['price'];
        $review_product_coverImages = $row['coverImage'];
    ...
    $review_valid_time = date('d-m-Y h:i:s a',strtotime($cart_update_time));
    $estimated_valid_return_time = date('D M d-m-Y', strtotime($review_valid_time . ' + 14 days') );//D=days星期几 M=month j=1-31号

    if($review_cart_variation == NULL){
        $review_cart_variation = " ";
    }else{
        $review_cart_variation = "Variation: ($review_cart_variation)";
    }

    if($review_cart_status == NULL){
        $review_cart_status = "Completed";
    }
}else{
    header("Location: ../user/404.html");
    die();
            ...
            <div class="col-md-8 changePassword-container">
                    ...
                    <div class="row m-3 shadow-sm p-3 bg-white">
                        ...
                        <a href="productDetail.php?productId=<?php echo $review_product_id;?>">                   
                                <img src="../images/productImage/<?php echo $review_product_coverImages;?>" alt="" class="w-100 h-100">
                            </a>
                        </div>
                        <div class="col-5">
                            <div class="col text-break">
                                <a class="text-decoration-none text-dark" href="productDetail.php?productId=<?php echo $review_product_id;?>">
                                    <?php echo $review_product_name;?></div>
                                </a>
                        ...
                        <div class="col-3">
                            <h5><span class="text-muted">RM:</span> <?php echo $review_product_price*$review_cart_qty;?></h5>
                ...
                <?php
                    $userId = $_SESSION['userId'];
                    $checkExist = "SELECT rating.*, comment.*, user.image, user.userName, rating.created_time AS 'ratingDate', cartintegration.variation FROM rating LEFT JOIN comment ON rating.commentId = comment.commentId LEFT JOIN user ON rating.userId = user.userId LEFT JOIN cartintegration ON rating.cartIntegrationId = cartintegration.cartIntegrationId WHERE rating.userId ='$userId' AND rating.cartIntegrationId = '$cartIntegrationId' LIMIT 1";
                    $resultCheckExist = $conn->query($checkExist);

                    if($resultCheckExist->num_rows>0){
                        while($row = $resultCheckExist->fetch_assoc()){
                            ...
                            $rating_user_image = $row['image'];
                            $rating_user_name = $row['userName'];
                            $rating_cartIntegration_variation = $row['variation'];

                            $rating_user_name = "Me";

                            if($rating_user_image == NULL){
                                $rating_user_image = "userIcon.png";
                            }

                            $ratingDate = date('d-m-Y h:i:s a',strtotime($rating_Date));
                ?>
                    <div class="row m-3 shadow-sm p-3 bg-white ">

                        <div class="post-container">
                            <img src="../images/profileImage/<?php echo $rating_user_image;?>" alt="user" class="profile-photo-md pull-left">
                            <div class="post-detail">
                            <div class="user-info">
                                <h5><a href="#" class="profile-link"><?php echo $rating_user_name;?></a> 
                                    <span class="following">
                                    <ul style="display:inline-block;">

                                    <?php
                                        $starNumber = $rating_value;
                                        for( $x = 0; $x < 5; $x++ )
                                        {
                                            if( floor($starNumber)-$x >= 1 ){ 
                                                echo '<i class="fa fa-star checked" style="font-size:15px"></i>'; 
                                            }else{ 
                                                echo '<i class="fa fa-star-o" style="font-size:15px"></i>'; 
                                            }
                                        }
                                    ?>    
                                        </ul>
                                ...
                                <p class="text-muted">Published a photo about <?php echo $ratingDate;?></p>
                            </div>
                            <!-- <div class="reaction">
                            ...
                            <div class="post-text">
                                <p><?php echo $rating_comment_text;?><i class="em em-anguished"></i> <i class="em em-anguished"></i> <i class="em em-anguished"></i></p>
                            </div>
                            <div class="post-comment">
                                <div class="row ml-2">
                                <?php 
                                    $checkExistMedia = "SELECT * FROM `feedback_image` WHERE feedback_sourceId = '$rating_comment_Id'";
                                    $resultCheckExistMedia = $conn->query($checkExistMedia);

                                    if($resultCheckExistMedia->num_rows>0){
                                        while($row = $resultCheckExistMedia->fetch_assoc()){
                                ...
                                ?>
                                    <div class="mr-2" style="width:15%">
                                        <img src="../images/feedback-images/<?php echo $filename;?>" alt="" class="w-100 h-100">
                                    </div>
                                <?php
                            ...
                            <div class="post-comment">
                                <div class="ml-2">
                                    <h5>Variation: <small><?php echo $rating_cartIntegration_variation;?></small></h5>         
                                </div>
                                <div class="col">
                                    ...
                                    <input type="hidden" id="EditPostRatingId" value="<?php echo $rating_Id;?>">
                                </div>
                            </div>
                            <div class="line-divider"></div>
                            <?php
                                $checkReply = "SELECT comment.*, user.userName, user.image AS 'userImage', seller.sellerName, seller.image AS 'sellerImage', comment.created_time AS 'replyDate'  FROM COMMENT LEFT JOIN user ON comment.comment_personId = user.userId LEFT JOIN seller ON comment.comment_personId = seller.sellerId WHERE comment.ratingId = '$rating_Id' AND comment.commentId != '$rating_comment_Id' ORDER BY comment.created_time DESC";
                                $resultCheckReply = $conn->query($checkReply);

                                    if($resultCheckReply->num_rows>0){
                                        while($row = $resultCheckReply->fetch_assoc()){
                                            $commnet_id = $row['commentId'];
                                            $comment_personid = $row['comment_personId'];
                                            $comment_Text = $row['commentText'];
                                            $comment_time = $row['created_time'];
                            ...
                            ?>
                            <div class="post-comment">
                                <img src="../images/profileImage/<?php echo $comment_userImage;?>" alt="" class="profile-photo-sm">
                                <p><a class="profile-link"><?php echo $comment_username;?> </a>  <span><?php echo $commentDate;?></span> <br> <i class="em em-laughing"></i> <?php echo $comment_Text;?></p>
                            </div>
                            <?php
                                            }else if($comment_username == NULL){
                            ?>
                            <div class="post-comment">
                                <img src="../images/profileImage/<?php echo $comment_sellerImage;?>" alt="" class="profile-photo-sm">
                                <p><a class="profile-link"><?php echo $comment_sellerName;?></a> <span><?php echo $commentDate;?></span> <br> <?php echo $comment_Text;?></p>
                            </div>
                            <?php
                                            }
                                        }

                                $getImage = "SELECT * FROM feedback_image WHERE feedback_sourceId = '$commnet_id'";
                            ...
                            ?>
                                        <div class="mr-2" style="">
                                            <video src="../images/feedback-images/<?php echo $row['feedback_location'];?>" controls width="140" height="100">
                                        </div>
                            <?php
                                        }else if($filetype == "image"){
                            ?>
                                        <div class="mr-2" style="">
                                            <img src="../images/feedback-images/<?php echo $row['feedback_location'];?>" alt="" class="" width="140" height="100">
                                        </div>
                            <?php
                                            }      
                                    }
                            ...
                            <?php
                                    ...
                                    $userInputBar = "SELECT image FROM user WHERE userId = '$userId'";
                                    $resultUserInputBar = $conn->query($userInputBar);

                                    if($resultUserInputBar->num_rows>0){
                                        while($row = $resultUserInputBar->fetch_assoc()){
                                            $userId_image = $row['image'];

                                            if(empty($userId_image)){
                                                $userId_image = "userIcon.png";
                                            }
                            ?>
                            <form method="POST" action="../database/user/insertNewComment.php?ratingId=<?php echo $rating_Id;?>&userId=<?php echo $userId;?>">
                                <div class="post-comment input-group mb-3">
                                    <div class="input-group" style="width:90%">
                                    <img src="../images/profileImage/<?php echo $userId_image;?>" alt="" class="profile-photo-sm">
                ...
                ?>
                    <!-- review and comment    -->
                    <form action="../database/user/reviewProcess.php" method="post"  enctype="multipart/form-data">
                        <div class="row m-3 shadow-sm p-3 bg-white">
                            <div class="col-12">
                                <h5 class="font-weight-bold">Review Information <small>(Maximun 6 pictures)</small></h5>
                                <input type="hidden" name="cartIntegrationId" value="<?php echo $cartIntegrationId;?>">
                                <input type="hidden" name="productId" value="<?php echo $review_product_id;?>">
                            </div>
                            <div class="col-12">
                                <div class="form-group mt-2" id="rating-ability-wrapper">
                                    ...
                                    <div class="row ml-1">
                                        ...
                                        <h2 class="bold rating-header ml-3" style="">
                                            <span class="selected-rating" style="font-size:35px">0</span><small> / 5</small>
                                        </h2>
                                    </div>
                                    <!-- End rating button -->
                            ...
                            <div class="col-12">
                                <textarea placeholder="  What do you think of the product?" name="commentText" cols="86" rows="5" id="checkChar"  minlength="5" maxlength="250" required  ></textarea>
                            </div>
                            <div class="col-12">
                        ...
                        <div class="row float-right mr-4">
                            <button type="submit" name="reviewInsert" class="btn btn-secondary btn-lg">Submit</button>
                        </div>
                    </form>
    ...
    <div class="modal fade" id="EditPostModal" tabindex="-1" role="dialog" aria-labelledby="EditPostModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            ...
            <div class="modal-header">
                <h5 class="modal-title" id="EditPostModalLabel">Edit Review</h5>
            ...
            <div class="modal-body m-3" id="EditPostContent">
                <div class="row">
                    <label class="control-label" for="rating">
                        <span class="field-label-header">According to your personal opinion, how many stars do you think the product can give? (1-5)</span><br>
                        <span class="field-label-info"></span>
                        <input type="hidden" id="modal_input_select_rating" name="selected_rating" value="" required>
                    </label>
                    <br>
                    ...
                    <h2 class="bold rating-header ml-3 " style="">
                        <span class="modal_selected-rating" style="font-size:35px">0</span><small> / 5</small>
                    </h2>
                </div>
                <div class="row mt-2">
                    <textarea placeholder="  What do you think of the product?" name="commentText" cols="86" rows="5" id="modalCheckChar"  minlength="5" maxlength="250" required ></textarea>        
                </div>
            ...
            <div class="modal-footer">
                ...
                <button type="button" name="updatePost" class="btn btn-primary">Save changes</button>
            </div>
            </div>
            </form>
...
$(document).ready(function() {
    ...
    $("#input-44").fileinput({
        ...
        allowedFileExtensions: ["jpg", "jpeg", "gif", "png", "mp3", "mp4", "wma","avi","3gp","mov","mpeg"],
        maxFilePreviewSize: 10240 //mb
    });
...
$('#EditPostBtn').click(function(){
    var id = $('#EditPostRatingId').val();
    $.ajax({
        url:"../database/user/EditPost.php",
        method:"POST",
        data:{ratingId:id}, //this is what data send between hyperlink
        success:function(data)
        {
        $('#EditPostContent').html(data); //show in this dynaimic_content place
        }
    }); 
    
    // modal edit rating
    $(".modalbtnrating").on('click',(function(e) {
        var modal_value = $('#modal_input_select_rating').val();
        var modal_selected_value = $(this).data("modal-attr");
        $('#modal_input_select_rating').val(modal_selected_value);

        //display
        $('.modal_selected-rating').empty();
        $('.modal_selected-rating').html(modal_selected_value);

        for (i = 1; i <= modal_selected_value; ++i) {
            $("#modal-rating-star-"+i).toggleClass('btn-warning');
            $("#modal-rating-star-"+i).toggleClass('btn-default');
        }
        
        for (ix = 1; ix <= modal_value; ++ix) {
            $("#modal-rating-star-"+ix).toggleClass('btn-warning');
            $("#modal-rating-star-"+ix).toggleClass('btn-default');
        }

    }));


    //check modal review lenght
    $('#modalCheckChar').keyup(function() {
        $('#modalCalChar').html(this.value.length);  
    });
